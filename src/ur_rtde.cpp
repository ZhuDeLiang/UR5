...
bool URRTDE::Implementation::setJoints(const RUT::VectorXd& joints) {
    assert(config.robot_interface_config.operation_mode ==
        RobotOperationMode::OPERATION_MODE_JOINT);
    std::vector<double> joint_positions(joints.data(), joints.data() + joints.size());
    return rtde_control_ptr->moveJ(joint_positions, config.linear_vel, config.linear_acc);
}
bool URRTDE::Implementation::getJoints(RUT::VectorXd& joints) {
    assert(config.robot_interface_config.operation_mode ==
        RobotOperationMode::OPERATION_MODE_JOINT);
    std::vector<double> joint_positions = rtde_receive_ptr->getActualQ();
    joints = RUT::VectorXd::Map(joint_positions.data(), joint_positions.size());
    return true;
}
